...
public class SendGuaranteeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SendGuaranteeController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SendGuaranteeController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (cus == null) {
            ...
            GuaranteeDAO gDAO = new GuaranteeDAO();
            request.setAttribute("glist", gDAO.getServiceTagByOderId(Integer.parseInt(orderID)));
            request.setAttribute("orderID", Integer.parseInt(orderID));
            request.setAttribute("productID", Integer.parseInt(productID));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        GuaranteeDAO gDAO = new GuaranteeDAO();
        request.setAttribute("glist", gDAO.getServiceTagByOderId(Integer.parseInt(oID)));
        request.setAttribute("orderID", Integer.parseInt(oID));
        request.setAttribute("productID", Integer.parseInt(pID));
        request.setAttribute("gDAO ", gDAO);
        request.setAttribute("user ", cus);
        String fileName = "";
        ...
        if (isImage(fileName)) {
            ...
            try {
                ...
                if (guaranteeDAO.CheckStatusInGuaranteeToAddGuarantee(serviceTag)) {
                    request.setAttribute("ERROR", "This product is under warranty period");
                    request.getRequestDispatcher("SendGuarantee.jsp").forward(request, response);
                } else {
                    if (orderID == 0) {
                        request.setAttribute("ERROR", "This product is not in order");
                        request.getRequestDispatcher("SendGuarantee.jsp").forward(request, response);
                    }
                    Date today = new Date();
                    ...
                    if (monthLeft < 0) {
                        request.setAttribute("ERROR", "Product has no longer guaranted");
                        request.getRequestDispatcher("SendGuarantee.jsp").forward(request, response);
                    } else {
                        guaranteeDAO.addNewGuarantee(productID, orderID, fileName, note, serviceTag);
                        response.sendRedirect("Guarantee");
                    }
            ...
            request.setAttribute("ERROR", "Please Upload Image File");
            request.getRequestDispatcher("SendGuarantee.jsp").forward(request, response);
        }
