...
public class ExportPDF extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            document.open();
            document.add(new Paragraph("Smart Tech Store", new Font(baseFont, 12)));
            document.add(new Paragraph("\n", new Font(baseFont, 8))); // Create a table with 3 columns
            PdfPTable table = new PdfPTable(1);
            table.setWidthPercentage(100);
            table.addCell(createCell("Warranty Certificate", false, 15));
            document.add(table);
            document.add(new Paragraph("\n", new Font(baseFont, 8)));
            ...
            table.setWidthPercentage(100);
            table.addCell(createCell("Customer Information", false, 10));
            table.addCell(createCell("Warranty period", false, 10));
            document.add(table);
            document.add(new Paragraph("\n", new Font(baseFont, 8)));
            table = new PdfPTable(2);
            table.setWidthPercentage(100);
            table.addCell(createCell("Creator: " + guaranteeDAO.getCreatorNameById(guarantee_record.getCreator_id()), false, 8));
            SimpleDateFormat date = new SimpleDateFormat("dd/MM/yyyy");
            table.addCell(createCell("Start Date:" + date.format(order.getOrder_date()), false, 8));
            table.addCell(createCell("Name:" + customer.getCus_name(), false, 8));
            table.addCell(createCell("Note:" + guarantee_record.getNote(), false, 8));
            table.addCell(createCell("Phone Number:" + customer.getPhone(), false, 8));

            table.addCell(createCell("Return Date:" + date.format(guarantee_record.getReturn_date()), false, 8));
            document.add(table);
            document.add(new Paragraph("\n", new Font(baseFont, 8)));
            table = new PdfPTable(1);
            table.setWidthPercentage(100);
            ...
            table.setWidthPercentage(100);
            table.addCell(createCell("Product", false, 8));
            table.addCell(createCell("Guarantee Period", false, 8));
            table.addCell(createCell("Type Of Defect", false, 8));
            document.add(table);
            table = new PdfPTable(1);
            table.setWidthPercentage(100);
            table.addCell(new PdfPCell());
        ...
        // Add "Customer Sign" cell
            table.addCell(createCell("Customer Sign", false, 9));
        // Create a cell for "Shop Sign" and set its alignment to right
            PdfPCell shopSignCell = createCell("Shop Sign", false, 9);
            ...
            response.getWriter().write("Error occurred during PDF generation: " + e.getMessage());
        }
    }
